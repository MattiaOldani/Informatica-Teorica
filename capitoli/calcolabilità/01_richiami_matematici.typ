...
- *locale/puntuale*: la funzione è definita solo per singoli valori $a$ e $b$ e scriviamo: $ f(a) = b quad bar.v quad a arrow.long.bar^f b, $ in cui $b$ è detta *immagine* di $a$ rispetto a $f$ e $a$ è detta *controimmagine* di $b$ rispetto a $f$.

Le funzioni possono essere categorizzate in base ad alcune proprietà:
- *iniettività*: una funzione $f: A arrow.long B$ si dice _iniettiva_ se e solo se: $ forall a_1, a_2 in A quad a_1 eq.not a_2 arrow.long.double f(a_1) eq.not f(a_2). $ In sostanza, elementi diversi sono mappati ad elementi diversi.
...
- *biettività*: una funzione $f: A arrow.long B$ si dice _biiettiva_ se e solo se: $ forall b in B quad exists! a in A quarter bar.v quarter f(a) = b, $ che equivale a dire che è sia iniettiva sia surriettiva.

Definendo l'*insieme immagine*: $ immagine(f) = {b in B quarter bar.v quarter exists a in A text("tale che") f(a) = b} = {f(a) quarter bar.v quarter a in A} subset.eq B, $ diamo una definizione alternativa di funzione suriettiva: una funzione è _suriettiva_ se e solo se $immagine(f) = B$.
...
Dato l'insieme $A$, la *funzione identità* su $A$ è la funzione $i_A: A arrow.long A$ tale che: $ forall a in A quad i_A (a) = a, $ quindi è una funzione che mappa ogni elemento su se stesso.

Grazie a questa, diamo una definizione alternativa di funzione inversa: data $f: A arrow.long B$ biiettiva, la sua inversa è l'unica funzione $f^(-1): B arrow.long A$ che soddisfa: $ f composizione f^(-1) = f^(-1) composizione f = id_A. $
...
- *totale* se è definita per _ogni_ elemento $a in A arrow.long.double forall a in A quad f(a) arrow.b$;
- *parziale* se è definita per _qualche_ elemento $a in A arrow.long.double exists a in A bar.v f(a) arrow.t$.
...
È possibile _totalizzazione_ una funzione parziale $f$ definendo una funzione a tratti $overline(f): A arrow.long B union {bot}$ tale che: $ overline(f)(a) = cases(f(a) quad & a in op("Dom")_f (a), bot & text("altrimenti")) quad . $

Il simbolo $bot$ introdotto è il simbolo di _indefinito_ e viene utilizzato per tutti i valori per cui la funzione di partenza $f$ non è appunto definita.
...
Dato che vogliamo che siano presenti anche tutte le funzioni parziali da $A$ in $B$, scriviamo: $ B^A_bot = { f : A -> B_bot }, $ mettendo in evidenza il fatto che tutte le funzioni che sono presenti sono totali oppure parziali, ma che sono state totalizzate. Le due definizioni coincidono.
...
- si tiene fisso $a$ e si provano tutte le funzioni $f$: si sta eseguendo dei _benchmark_, ognuno dei quali è rappresentato da $a$;
- si tiene fissa $f$ e si provano tutte le $a$ nel dominio: si ottiene il grafico di $f$.
