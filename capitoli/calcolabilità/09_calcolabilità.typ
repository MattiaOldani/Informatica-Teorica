...
#figure(image("assets/composizione.svg", width: 50%))
...
#figure(image("assets/calcolabilità-venn.svg", width: 100%))
...
#theorem-proof()[
  ...
  #figure(image("assets/funzione-stato-prossimo.svg", width: 70%))
  ...
  - *passi induttivi*: i comandi "complessi" WHILE devono stare in $cal(P)$.
    ...
    In conclusione, $ f_(C') (x) = f_C^e(x) = T(x,e(x)) $ è composizione di funzioni in $cal(P)$, quindi $f_(C') in cal(P)$. #qedhere
]
