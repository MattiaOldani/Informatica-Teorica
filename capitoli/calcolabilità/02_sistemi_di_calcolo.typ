...
Un *sistema di calcolo* lo possiamo vedere come una _black-box_ che prende in input un programma $P$, dei dati $x$ e calcola il risultato di $P$ su input $x$.

La macchina ci restituisce $y$ se è riuscita a calcolare un risultato, $bot$ se è entrata in un loop.
...
Come vediamo, è molto simile ad una funzione di valutazione, infatti la parte dei dati $x$ corrisponde all'input $a$, mentre potremmo mappare il programma $P$ alla funzione $f$. Ci serve, però, definire cos'è un programma.
...
Con $cal(C)(P,x)$ indichiamo la funzione calcolata da $P$ su $x$, che viene detta *semantica*, il suo "significato".
...
Detto ciò, definiamo la *potenza computazionale* di un calcolatore come l'insieme di tutte le funzioni che quel sistema di calcolo può calcolare grazie ai programmi: $ F(cal(C)) = { cal(C)(P, \_) bar.v P in programmi } subset.eq dati_bot^dati. $
...
- se $F(cal(C)) subset.neq dati_bot^dati arrow.double.long$ esistono compiti *non automatizzabili*;
- se $F(cal(C)) = dati_bot^dati arrow.double.long$ l'informatica può fare tutto.
...
Risolvere un problema equivale a calcolare una funzione: ad ogni problema è possibile associare una *funzione soluzione*, che mi permette di risolverlo automaticamente.\
